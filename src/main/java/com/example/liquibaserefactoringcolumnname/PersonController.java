...
public class PersonController {
    ...
    public Person createPerson(@RequestBody CreatePersonCommand command) {
        jdbcTemplate.update("INSERT INTO person (lastname, firstname, state) VALUES (?, ?, ?)",
                command.getLastName(), command.getFirstName(), command.getState().name());
        return jdbcTemplate.queryForObject("SELECT id, lastname, firstname, state FROM person WHERE id = (SELECT currval('person_id_seq'))", new PersonRowMapper());
    }

    @PutMapping("/{id}")
    public Person updatePerson(@PathVariable("id") int id, @RequestBody UpdatePersonCommand command) {
        jdbcTemplate.update("UPDATE person SET lastname = ?, firstname = ?, state = ? WHERE id = ?",
